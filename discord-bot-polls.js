...
const server = http.createServer(async (req, res) => {
  ...
  // Endpoint pour le calendrier/planning
  else if (req.method === 'POST' && req.url === '/api/calendar/post') {
    let body = '';
    req.on('data', chunk => {
      body += chunk.toString();
    });
    req.on('end', async () => {
      try {
        const data = JSON.parse(body);
        const { channelId, message, sessionId, action } = data;
        
        console.log('📅 Notification calendrier reçue:', { channelId, sessionId, action });
        ...
        if (token !== process.env.DISCORD_API_KEY) {
        ...
        const embed = new EmbedBuilder()
          .setColor(action === 'delete' ? 0xFF0000 : 0xFFA500)
          .setDescription(message)
          .setTimestamp()
          .setFooter({ text: 'ClubStats Pro • Planning' });
        
        // Envoyer le message
        const sentMessage = await channel.send({ embeds: [embed] });
        
        // Ajouter les réactions (sauf si suppression)
        if (action !== 'delete') {
          await sentMessage.react('✅');
          await sentMessage.react('❌');
        }
        
        console.log('✅ Message calendrier posté dans #' + channel.name);
        ...
        res.end(JSON.stringify({ error: 'Failed to post calendar message' }));
      }
    });
  } else {
    // Keep-alive endpoint
